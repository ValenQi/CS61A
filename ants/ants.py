...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.number == 1:
                return
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        """Returns True if and only if:
           1. SELF is a container 2. SELF does not already contain another ant 
           3. OTHER is not a container
        """
...
class WallAnt(Ant):
    """A WallAnt does nothing each turn but has a large value."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is watersafe and do not lose armor when placed in Water."""
...
class LaserAnt(ThrowerAnt):
    """A LaserAnt hits all ants and bees in front of it with its powerful laser.
    The laser gets weaker as it hits more ants or the farther away it is from the target.
    ...
    def demage_done(self, insect, Hove):
        lasered_place, distance, count = self.place.entrance, 1, 0
        while lasered_place != Hove:
            if lasered_place == insect.place:
                return self.damage - distance_weakening * distance - insects_shot_weakening * count
            
            lasered_place = lasered_place.entrance
            distance += 1
            number_of_ants = 0
            if lasered_place.ant:
                if isinstance(lasered_place.ant, BodyguardAnt) and lasered_place.ant.ant:
                    number_of_ants += 1
                number_of_ants += 1
            count += len(lasered_place.bees) + number_of_ants
        return 0
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        self.number = QueenAnt.total_queens
        self.doubled_ants = []
    ...
    def action(self, colony):
        ...
        """
        if self.number != 1:   # imposter queens
            self.reduce_armor(self.armor)
        else:
            ScubaThrower.action(self, colony)
            same_tunnel_ants = []
            # append queen's bodyGuard to same_tunnel_ants if she has any
            if isinstance(self.place.ant, BodyguardAnt):
                same_tunnel_ants.append(self.place.ant)

            def next_place(curr_place, i):
                """Return the exit of CURR_PLACE if tracing towards the end of the tunnel.
                   Return the entrance if tracing in the opposite direction"""
                return curr_place.entrance if i else curr_place.exit
            # collect all ants in the same tunnel as queen
            places = [self.place.exit, self.place.entrance]
            for i in range(2):
                while places[i]:
                    if places[i].ant:
                        same_tunnel_ants.append(places[i].ant)
                        if isinstance(places[i].ant, BodyguardAnt):   
                            same_tunnel_ants.append(places[i].ant.ant) 
                    places[i] = next_place(places[i], i)
            # double ants' damages if necessary
            for ant in same_tunnel_ants:
...
def make_slow(action):
    ...
    """
    def slowed_method(colony):
        if colony.time % 2 == 0:
            return action(colony)
        return None
    return slowed_method
...
def make_stun(action):
    ...
    action -- An action method of some Bee
    """
    def stunned_method(colony):
        return None
    return stunned_method
