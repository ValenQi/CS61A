...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    try:
        ...
        raise SchemeError("handling a: {0}".format(type(e)))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("number of symbols unequal to number of values")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(symbol, LambdaProcedure(formals, expressions.second, env))
        return symbol
    else:
...
def do_if_form(expressions, env):
    ...
    if scheme_false(scheme_eval(expressions[0], env)):
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        return True
    while len(expressions) > 1:
        result, expressions = scheme_eval(expressions.first, env), expressions.second
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formal, val = binding.first, scheme_eval(binding[1], env)
...
def check_formals(formals):
    ...
    while formals is not nil:
        formal, formals = formals.first, formals.second
        if not scheme_symbolp(formal):
            raise SchemeError("formals contains invalid symbol: {0}".format(formal))
        if formal in lst:
            raise SchemeError("list of formals contains repeated symbols")
        else:
            lst += formal
