...
'(1 . 2 2)
; expect Error: Expected one element after .
...
(begin (define y (+ x y)) (* x 10))
; expect 30

y
; expect 7

(begin (define sum (+ x y)) nil)
; expect ()
...
;--------------------------------------
; test for both problem 10 and 12
(define (f x)
  (define (g y)
    (+ x y)
  )
...
;--# 11------------------------------------
(lambda (y x z) (+ x x))
; expect (lambda (y x z) (+ x x))

(lambda (y y z) (+ y z))
; expect Error: list of formals contains repeated symbols

(lambda (1) ("should not print"))
; expect Error: formals contains invalid symbol: 1
...
(define return_true (lambda () true))
; expect return_true
(if (return_true) 'true 'false)
; expect True
...
;--#15------------------------------------
(cond ('return_predicate))
; expect return_predicate

(cond ((= 1 1) 'correct)
      ((= 1 1) (/ 1 0)))
; expect correct

(cond ((= (+ 1 1) 3) 'incorrect)
      ((= (+ 1 1) 2) 'correct))
; expect correct

(cond ((< 1 2) (define x 1) (+ x x))
      (else 'incorrect))
;expect 2
...
;--#17------------------------------------
(define func_mu (mu (x y) (+ x y unknown)))
; expect func_mu
(define func_lambda (lambda (n unknown) (func_mu n n)))
; expect func_lambda
