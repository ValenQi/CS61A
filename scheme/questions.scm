...
(define (list-partitions total)
    (define (partitions n max used)
        (cond ((= n 0) (cons nil nil))
              ((or (< n 0) (= max 0)) nil)
              (else (let ((use-max (partitions (- n max) max max))
                          (without-max (partitions n (- max 1) used)))
                         (if (= max (- used 1))
                             without-max            
                             (append (cons-all max use-max) without-max)
                         )))))
    (partitions total total 0))
  ...
  (cond ((atom? expr) 
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
              (begin (define params (apply-to-all analyze (car (zip values))))
                     (define args   (apply-to-all analyze (cadr (zip values))))
                     (cons (cons 'lambda (cons params (analyze body)))
                           args))))
        (else 
         (let ((form   (analyze (car expr)))
               (params (apply-to-all analyze (cdr expr))))
           (cons form params)))))
...
(analyze '(let ((a 10))
            (let ((b a))
              b)))
; expect ((lambda (a) ((lambda (b) b) a)) 10)
(analyze '(+ 1 (let ((a 1)) a)))
